...
module Synapse
  class ServersetWatcher < BaseWatcher
    NUMBERS_RE = /^\d+$/

    def start
      zk_hosts = @discovery['hosts'].shuffle.join(',')

      log.info "synapse: starting serverset watcher #{@name} @ hosts: #{zk_hosts}, path: #{@discovery['path']}"
      @should_exit = false
      @zk = ZK.new(zk_hosts)
    ...
    def stop
      log.warn "synapse: serverset watcher exiting"

      @should_exit = true
      @watcher.unsubscribe if defined? @watcher
      @zk.close! if defined? @zk

      log.info "synapse: serverset watcher cleaned up successfully"
    end
    ...
    def validate_discovery_opts
      raise ArgumentError, "invalid discovery method #{@discovery['method']}" \
        unless @discovery['method'] == 'serverset'
    ...
    def deserialize_service_instance(data)
      ...
      host = serviceEndpoint['host'] || (raise ValueError, 'instance json data does not have host key')
      port = serviceEndpoint['port'] || (raise ValueError, 'instance json data does not have port key')
      name = serviceEndpoint['name'] || nil
